      ...
      >
        <div class="map__list__box">
          <ul class="map__ul">
            <h1 class="search__title">search list</h1>
            <div class="orderBtn">
              <b-button @click="orderByName(aparts)">이름 순 정렬</b-button>
              <b-button @click="orderByPrice(aparts)">가격 순 정렬</b-button>
              <b-button @click="orderByDate(aparts)">날짜 순 정렬</b-button>
            </div>
            <li v-for="(item, i) in aparts" :key="i">
              <apart-item
                :apartInfo="item"
      ...
      >
        <div class="map__list__box">
          <ul class="map__ul">
            <h1 class="search__title">My like list</h1>
            <div class="orderBtn">
              <b-button @click="orderByNameLike(myLikesApt)">이름 순 정렬</b-button>
              <b-button @click="orderByPriceLike(myLikesApt)">가격 순 정렬</b-button>
              <b-button @click="orderByDateLike(myLikesApt)">날짜 순 정렬</b-button>
            </div>
            <li v-for="(item, i) in myLikesApt" :key="i">
              <like-apart-item
                :apartInfo="item"
  ...
  methods: {
    ...
    orderByPrice(apt) {
      if (this.orderPrice === false) {
        apt.sort((a, b) => {
          return Number(a.apartPrice.trim()) - Number(b.apartPrice.trim());
        });
        this.orderPrice = true;
      } else {
        apt.sort((a, b) => {
          return Number(b.apartPrice.trim()) - Number(a.apartPrice.trim());
        });
    ...
    orderByName(apt) {
      if (this.orderName === false) {
        apt.sort((a, b) => {
          return a.apartName < b.apartName ? -1 : a.apartName > b.apartName ? 1 : 0;
        });
        this.orderName = true;
      } else {
        apt.sort((a, b) => {
    ...
    orderByDate(apt) {
      if (this.orderDate === false) {
        apt.sort((a, b) => {
          if (a.TradeDate.year === b.TradeDate.year) {
            if (a.TradeDate.month === b.TradeDate.month) {
              return a.TradeDate.day - b.TradeDate.day;
            } else {
              return a.TradeDate.month - b.TradeDate.month;
            }
          }
          return a.TradeDate.year - b.TradeDate.year;
        });
        this.orderDate = true;
      } else {
        apt.sort((a, b) => {
          if (b.TradeDate.year === a.TradeDate.year) {
            if (b.TradeDate.month === a.TradeDate.month) {
              return b.TradeDate.day - a.TradeDate.day;
            } else {
              return b.TradeDate.month - a.TradeDate.month;
            }
          }
          return b.TradeDate.year - a.TradeDate.year;
        });
        this.orderDate = false;
      }
    },
    orderByPriceLike(apt) {
      if (this.orderPriceLike === false) {
        apt.sort((a, b) => {
          return (
            Number(a.info.price.replace(",", "").trim()) -
            Number(b.info.price.replace(",", "").trim())
          );
        });
        this.orderPriceLike = true;
      } else {
        apt.sort((a, b) => {
          return (
            Number(b.info.price.replace(",", "").trim()) -
        ...
        this.orderPriceLike = false;
      }
    },
    orderByNameLike(apt) {
      if (this.orderNameLike === false) {
        apt.sort((a, b) => {
          return a.info.apartname < b.info.apartname
            ? -1
            : a.info.apartname > b.info.apartname
            ? 1
            : 0;
        });
        this.orderNameLike = true;
      } else {
        apt.sort((a, b) => {
          return a.info.apartname > b.info.apartname
            ? -1
        ...
        this.orderNameLike = false;
      }
    },
    orderByDateLike(apt) {
      if (this.orderDateLike === false) {
        apt.sort((a, b) => {
          return a.info.date.replaceAll("-", "") - b.info.date.replaceAll("-", "");
        });
        this.orderDateLike = true;
      } else {
        apt.sort((a, b) => {
          return b.info.date.replaceAll("-", "") - a.info.date.replaceAll("-", "");
        });
        this.orderDateLike = false;
      }
    },
    bk() {
      this.currCategory = "BK9";
